...
namespace boxpp
{
	namespace _ {
		...
		> struct EqualsImpl {
			...
			FASTINLINE static bool Compare(Type& Left, const Type& Right) {
				return Left == Right;
			}
		};
		...
		struct OperatorOrMemcmp
		{
			...
			FASTINLINE static s32 CStyleCompare(Type& Left, const Type& Right) {
				return ::memcmp(&Left, &Right, sizeof(Type));
			}
		};
	...
	template<typename Type>
	FASTINLINE s32 Compare(Type& Left, const Type& Right)
	{
