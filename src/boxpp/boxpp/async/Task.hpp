...
namespace boxpp {
	namespace async {
		...
		class TTask
		{
			...
			friend class TTaskSource;

		public:
			TTask(nullptr_t = nullptr) { }
			TTask(const TTask<ReturnType>& Task) : TaskSource(Task.TaskSource) { }
			TTask(TTask<ReturnType>&& Task) : TaskSource(TMovable<TTaskSourcePtr<ReturnType>>::Movable(Task.TaskSource)) { }
			TTask(const TTaskSourcePtr<ReturnType>& TaskSource)
		...
		private:
			TTaskSourcePtr<ReturnType> TaskSource;
		...
		public:
			...
			FASTINLINE TTask<ReturnType>& operator =(TTask<ReturnType>& Task) {
				TaskSource = TMovable<TTaskSourcePtr<ReturnType>>::Movable(Task.TaskSource);
				return *this;
			}

			FASTINLINE TTask<ReturnType>& operator =(const TTaskSourcePtr<ReturnType>& TaskSource) {
